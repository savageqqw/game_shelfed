...
export default withErrors(async (req, res) => {
  ...
  const { game_id, title, cover, status } = req.body || {}
  ...
  await db.execute({
    sql: `INSERT INTO library_items (user_id, game_id, title, cover, status, updated_at)
          VALUES (?, ?, ?, ?, ?, datetime('now'))
    ...
    args: [user.id, String(game_id), title, cover || null, status]
  })
